...
const UploadVideo = () => {
  ...
  useEffect(() => {
    if (location.state?.conteudoId) {
      ...
      setModuloId(location.state?.moduloId || null);
      ...
      return new Promise((resolve, reject) => {
        const upload = new tus.Upload(currentVideo.file, {
          ...
          onSuccess: async () => {
            try {
              await axios.post(
                `https://api.digitaleduca.com.vc/vimeo-client/video/${dbVideoId}/update-metadata`,
  ...
  const handleSubmit = async () => {
    ...
    if (videosWithoutTitle.length > 0) {
      return showMessage("Todos os vídeos devem ter um título.", true);
    }
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body1" color="text.secondary">
                    Adicione vídeos ao seu conteúdo
                  </Typography>
                </Box>
              </Box>
